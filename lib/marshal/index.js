...
var TYPES = {
  ...
  'MapType': { ser:Serializers.encodeBinary, de:Deserializers.decodeMap },
  'SetType': { ser:Serializers.encodeBinary, de:Deserializers.decodeList },
  'ListType': { ser:Serializers.encodeBinary, de:Deserializers.decodeList }
};
...
function getMapType(str){
  var types = str.slice(str.indexOf('(') + 1, -1).split(',');
  types[0] = getType(types[0]);
  types[1] = getType(types[1]);
  return {
	  name: 'MapType',
	  type: types
  };
...
function getSetType(str){
	var type = str.slice(str.indexOf('(') + 1, -1);
	type = getType(type);
	return {
		name: 'SetType',
		type: type
...
function getDeserializer(type, subtype){
  ...
  if (Array.isArray(type)){
    ...
    return function(val) {
      return val !== null ? TYPES[type].de.apply(TYPES, [val, subtype]) : null;
    };
  }
}
