...
function createCollectionGetter(typeName)
{
    return function getCollectionType(string)
    {
...
var getSetType = createCollectionGetter('SetType'),
    getListType = createCollectionGetter('ListType');
 ...
 * @memberOf Marshal
 */
function getSerializer(type, subtype)
{
    if (Array.isArray(type))
        return compositeSerializer(_.map(type, function(value) { return getSerializer(value, subtype); }));

    return function serializer(val)
    {
        return Serializers[type.replace(/^\s+|\s+$/g,'')](val, subtype);
    };
}
...
var Marshal = function Marshal(type)
{
    ...
    this.serialize = getSerializer(parsedType, this.type);
