...
function getMapType(str){
  var map_types = str.slice(str.indexOf('(') + 1, -1).split(',');
  map_types[0] = getType(map_types[0]);
  map_types[1] = getType(map_types[1]);
  return {
	  name: 'MapType',
	  types: map_types
  };
 ...
 * @memberOf Marshal
 */
function getDeserializer(type, subtypes){
  if (Array.isArray(type)){
    ...
    return function(val) {
      return val !== null ? TYPES[type].de.apply(TYPES, [val, subtypes]) : null;
    };
  }
}
