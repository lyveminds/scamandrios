...
Deserializers.decodeMap = function(val, types)
{
    ...
    // Skip the cardinality.
    var offset = 2;

    for (var byteLength = value.length; offset < byteLength;)
    {
...
Deserializers.decodeList = function(val, type)
{
    var value = new Buffer(val, 'binary'),
        offset = 0,
        cardinality = value.readUInt16BE(offset);

    offset += 2;

    var list = Array(cardinality),
        index = cardinality;

    for (var byteLength = value.length; offset < byteLength; index--)
    {
        var elementLength = value.readUInt16BE(offset),
            element = value.slice(offset += 2, offset += elementLength);

        list[cardinality - index] = module.exports[type](element);
    }
...
Deserializers.decodeDottedQuad = function(val)
{
  if (!val)
    return null;

  var quads = Buffer.isBuffer(val) ? val : new Buffer(val, 'binary');
  return _.map(quads, function(i) { return String(i); }).join('.');
};
...
module.exports =
{
    ...
    'TimestampType': Deserializers.decodeDate,
    ...
    'SetType': Deserializers.decodeList,
    'ListType': Deserializers.decodeList,
    'InetAddressType': Deserializers.decodeDottedQuad
};
