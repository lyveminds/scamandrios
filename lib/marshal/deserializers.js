...
/**
 * Decode a map into an object
 * @param {String}	val		The binary string to decode
 * @param {Array}	types	The types for the fields of the map
 * @returns {Object} Key-value pairs representing the map
 */
Deserializers.decodeMap = function(val, types) {
	...
	var offset = 2;
	var key, key_len;
	var map_value, value_len;

	var value = new Buffer(val, 'binary');
	while(1) {

		// Read the key
		key_len = value.readUInt16BE(offset);
		key = value.slice(offset + 2, (offset + 2 + key_len));
		key = this[types[0]].de(key);

		// Advance to read the value
		offset = offset + 2 + key_len;
		value_len = value.readUInt16BE(offset);
		map_value = value.slice(offset + 2, (offset + 2 + value_len));
		map_value = this[types[1]].de(map_value);
...
/**
 * Decode a LIST or SET into an Array
 * @param {String}	val
 * @param {String} type
 */
Deserializers.decodeList = function(val, type) {
	...
	var list = [];
	var offset = 2;
	var elem_len, elem_value;
	var value = new Buffer(val, 'binary');

	while(1) {

		// first two bytes (short) are the length of the element
		elem_len = value.readUInt16BE(offset);

		// add 2 to get over the size, then read elem_len bytes
		elem_value = value.slice(offset + 2, (offset + 2 + elem_len));

		// Add it to the list
		list.push(this[type].de(elem_value));

		offset = offset + elem_len + 2;
		if(offset >= value.length) {
			break;
		}
	}
