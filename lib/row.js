...
var Row = function(data, schema){
  ...
  var row = [], i = 0, len = data.columns.length, item, name,
  ...
  for(; i < len; i += 1){
    ...
    if(item.name !== 'KEY'){
      ...
      if(schema.noDeserialize) {
        row.push(new Column(name,item.value, new Date(item.timestamp / 1000), item.ttl));
      } else {
        row.push(new Column(name,deserializeValue(item.value), new Date(item.timestamp / 1000), item.ttl));
      }

      row._map[name] = row.length - 1;
    }
  }
...
Object.defineProperty(Row.prototype, 'count',
{
    'get': function get()
    {
        return this.length;
...
Row.prototype.slice = function(start, end){
  ...
  var matches = [].slice.call(this, start, end);
  return new Row({ key:this.key, columns:matches }, this._schema);
};
