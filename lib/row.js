...
Row.prototype.toString = Row.prototype.inspect = function toString()
{
    ...
    var columns = [];
    _.forOwn(data.nameToIndex, function(columnIndex, name)
    {
        columns.push(name);
    });

    var key = Array.isArray(data.key) ? data.key.join(':') : data.key;
...
{
    var data = metadata.get(this);
    return data ? this[data.nameToIndex[name]] : null;
};
...
{
    var data = metadata.get(this),
        columns = slice.call(this, start, end);

    return data ? new Row({ 'key': data.key, 'columns': columns }, data.schema) : columns;
};
...
Row.prototype.nameSlice = function nameSlice(start, end)
{
    ...
    _.forOwn(data.nameToIndex, function(index, key)
    {
        if (key >= start && key < end)
            columns.push(self[index]);
    });

    return new Row({ 'key': data.key, 'columns': columns }, data.schema);
};
