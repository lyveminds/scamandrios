...
var Pool = module.exports = function Pool(options)
{
    ...
    var hosts = this.hosts = 'hosts' in options || !('host' in options) ? options.hosts : [options.host];
...
_.extend(Pool.prototype,
{
    ...
    'connect': function connect()
    {
        ...
        var handleReject = _.partial(settlePromise, false);
        function settlePromise(isResolved, value)
        {
            pendingCount--;
            if (isResolved)
        ...
        _.forEach(hosts, function (host)
        {
            _.times(poolSize, function ()
            {
                ...
                connection.then(function (keyspace)
                {
                    settlePromise(true, keyspace);
                    if (keyspace)
    ...
    'monitorConnections': function monitorConnections()
    {
        ...
        function checkDead()
        {
            ...
            {
                var host = dead.pop(),
                    connection = self.connectTo(host);
