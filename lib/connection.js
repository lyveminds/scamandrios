...
Connection.prototype.selectCqlVersion = function selectCqlVersion()
{
    ...
    this._client.describe_version(function (error, version)
    {
        ...
        if (self.keyspace == null)
            return deferred.resolve();
        self.use(keyspace).then(deferred.resolve);
    });
    return deferred.promise;
};

Connection.prototype.use = function use(keyspace)
{
    var deferred = P.defer(),
        self = this;
...
Connection.prototype.cql = function cql(cmd, args, options)
{
    ...
    options = options || {};
    var cql = new Buffer(args ? formatCQL(cmd, args) : cmd);

    var useGzip = options.gzip === true,
        query = (useGzip ? createDeflate : P)(cql);
...
Connection.prototype.createKeyspace = function createKeyspace(keyspace, options)
{
    ...
    if (!keyspace)
        return deferred.reject(errors.create({ 'name': 'InvalidNameError', 'why': 'Keyspace name not specified' }));
    var args =
    {
        'name': keyspace,
...
Connection.prototype.dropKeyspace = function dropKeyspace(keyspace, options)
{
    ...
    if (!keyspace)
        return deferred.reject(errors.create({ 'name': 'InvalidNameError', 'why': 'Keyspace name not specified' }));
    this._client.system_drop_keyspace(ksdef, function (error, response)
    {
        if (error)
            return deferred.reject(errors.create(error));
        deferred.resolve(response);
