...
Connection.prototype.connectionLifespan = null;
...
Connection.prototype.connect = function connect()
{
    ...
    connection.on('connect', function (error)
    {
        ...
        // We let this connection live for 20 minutes before tearing it down & rebuilding it.
        if (self.connectionLifespan)
            clearTimeout(self.connectionLifespan);
        self.connectionLifespan = setTimeout(self.refreshConnection.bind(self), self.TTL || Connection.TTL);
...
{
    if (this.connectionLifespan)
        clearTimeout(this.connectionLifespan);
